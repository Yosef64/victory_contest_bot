
welcom_text = r"""
🎉 Hey there\, Champion\! Welcome to Victory Contest\! 🎉

We’re thrilled to have you here\! 🚀 This is not just any contest—this is your chance to show off your skills\, challenge yourself\, and rise to the top\! 🏆

👀 What’s waiting for you?
✅ Exciting challenges that test your knowledge \& speed
✅ A chance to compete with brilliant minds like you
✅ Leaderboards\, rewards\, and all the bragging rights\!

💡 But first… You need to register\!
Just tap the button below\, and you’ll be all set to jump into the action\.

👇 Hit the button \& let the games begin\! 👇

We can’t wait to see you in action\! 🎯🔥
"""
welcome_back_txt = r"""
🔥 Welcome back\, Champion\! 🔥

Great to see you again\! 🎉 We know you’re here to win\, learn\, and level up\, and we’re ready to challenge you\!

🚀 What’s next?
✅ Jump back into the contest and keep climbing the leaderboard 🏆
✅ Test your skills with new challenges 💡
✅ Stay ahead of the competition \& claim your spot at the top\!

Are you ready? Let’s get back in the game\! 🎯💥
"""
error_message = (
    r"""⚠️ *Oops\, something went wrong\!* 😢\n\n
    🔍 *Possible reasons\:*\n"
    ❌ Invalid request or command not recognized\.\n
    🔌 Connection issue\, please try again\.\n
    ⏳ Server might be busy\, hold on a moment\.\n\n"""
)
rules = (
    r"""📜 *Contest Rules \& Guidelines* ⚖️\n\n
    🚀 *Ready to compete? Make sure you follow these rules\!*\n\n
    1️⃣ *One Account\, One Contest!* ❌🆕\n"
       \- You *CANNOT* enter the contest using *multiple accounts* at the same time\.\n\n
    2️⃣ *No Late Entries\!* ⏳🚫\n
       \- Once the contest *ends*\, you *cannot access it* anymore\.\n\n
    3️⃣ *Registration is Required\!* 📝✅\n
       \- Before joining\, you *must register* for the contest\.\n\n
    ⚡ *Follow the rules and compete fairly\!* Breaking any of these rules may result in *disqualification*\. 🚀\n\n"""
   
)